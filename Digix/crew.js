...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '22652839161@s.whatsapp.net'; // ton numéro ou le groupe cible
                ...
                const messageText = `
╔══════════════════╗
      *yiwalo Bot Connected Successfully* 🚀
╠══════════════════╣
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 22652839161; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '22652839161' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'YIWALODI');
                console.log('📲 Pairing Code:', code);
                console.log('👉 Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['22652839161@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
